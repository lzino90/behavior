function [z,zz,zzz,zzzz,R,u,expo,prev] = SIR_gameN(n,A,m,a, lambda, mu,beta,sigma,c,gamma,u_func, r_func,T, init)
%n: number of nodes
...
for t=2:T+1
    ...
    W=ADN(n,m,a);
    ip=(1-(1-lambda).^(W*((x==1)')))'; %exposed not infectious
    
    x=x+(rand(1,n)<=ip.*(x==0).*(1-sigma*y))+(rand(1,n)<mu*(x==1));
    z(t)=sum((x==1))/n;
...
plot(0:T,zz)
ylim([0 1])
legend('Prevalence of Disease','Recovered','Fraction Self-Isolating')
% plot(0:T,zzzz)
