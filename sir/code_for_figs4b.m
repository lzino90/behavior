...
var=a2-a1^2;
f = @(sol)var/a1^2-gamma(1+2./sol)./gamma(1+1./sol).^2+1;
sol0 = 1;               % Initial guess
sol = fzero(f,sol0)  ;     % Solve for k
lam = a1/gamma(1+1/sol);
a = wblrnd(lam,sol,1,n);
...
u_in=[6, 0.5];       %Vector [Control law, Control effort]
...
[z,zz,zzz,zzzz,R,u]= SIR_gameN(n,A,m,a, lambda, mu,beta,c,gamma,u_in, f,T, init);
